...
def add_note(nokogiri_children, content, part)
...
def parse_content(content, clause_no, content_type = 'guidance')
  ...
  body.children.each do |c|
    ...
    contains_alink        = c.search('a').count > 0
    contains_colon_at_end = c.content.split(//).last(1).join == ':'
    ...
    if is_olist
      ...
      list_items.each do |item|
        ...
        if item.search('a').count > 0
          content = replace_link(item, content)
        end

        if item.search('.note').count > 0
...
class Catalog
  ...
  def hashify_catalog_groups
    @catalog['groups'].map!{ |g| g.to_hash }
  end
...
File.open("scripts/output/iso27002-oscal.yml", "w") { |file|
  file.write(oscal_catalog.to_hash.to_yaml)
}
